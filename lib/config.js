	...
	var readInnerConfig = function (name, innerConfig) {
		...
		for (var prop in innerConfig) {
			if (prop === '_handler') {
				...
				if (innerConfig._log.filePath) {
					loggerInfo.streams.push({
						path: getPath(innerConfig._log.filePath, configDir)
					});
				}
				if (innerConfig._log.stdout) {
					loggerInfo.streams.push({
						stream: process.stderr,
						level: 'info'
					});
				}
				var logger = bunyan.createLogger(loggerInfo);
				retInner.log = logger;
			}
			else if (prop.indexOf('_env') === 0)
			{
				var value = process.env[innerConfig[prop]];
